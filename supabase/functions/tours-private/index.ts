...
serve(async (req) => {
  ...
  try {
    if (req.method === 'GET') {
      const { data, error } = await supabase.from('tours_private').select('*').order('created_at', { ascending: false });
      if (error) return err(error.message, 400);
      return ok(data);
    }
    if (req.method === 'POST') {
      const body = await req.json();
      const { data, error } = await supabase.from('tours_private').insert(body).select().single();
      if (error) return err(error.message, 400);
      return ok(data);
    }
    if (req.method === 'PUT') {
      const body = await req.json(); const { id, ...fields } = body;
      const { data, error } = await supabase.from('tours_private').update(fields).eq('id', id).select().single();
      if (error) return err(error.message, 400);
      return ok(data);
    }
    if (req.method === 'DELETE') {
      const id = url.searchParams.get('id'); if (!id) return err('Missing id', 400);
      const { error } = await supabase.from('tours_private').delete().eq('id', id);
      if (error) return err(error.message, 400);
      return ok({ ok: true });
    }
