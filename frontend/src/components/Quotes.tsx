...
type QuoteItem = {
  ...
  pricing_type?: "per_pax" | "per_group";
...
export function Quotes() {
  ...
  useEffect(() => {
    (async () => {
      ...
      setQuotes(data || []);
      if ((data || []).length) {
        void selectQuote((data as Quote[])[0]);
      }
    })();
  }, []);
    ...
    return matrixPax.map((pax) => {
      const net = items.reduce((acc, it) => {
        ...
        if (it.pricing_type === "per_group") {
          ...
          // 1 free per 20 paying (hotels only)
          const paying =
            it.service_type === "hotel"
              ? Math.max(pax - Math.floor(pax / 20), 0)
              : pax;
  ...
  function generatePdf(lang: "en" | "he" = "en") {
    ...
    if (active) {
      const rows = [
        ...
        `${rtl ? "תאריכים" : "Dates"}: ${active.start_date || ""} → ${
          active.end_date || ""
        }`,
      ];
    ...
    let y = 160;
    doc.setFontSize(12);
    doc.text(rtl ? "מטריצת מחירים (נוסעים)" : "Price Matrix (PAX)", 40, y);
    y += 14;
    matrix.forEach((r) => {
      doc.text(
        `${r.pax} → Net ${r.net}  |  Gross ${r.gross}  |  PP ${r.pp}`,
        40,
        y
      );
      y += 16;
  ...
  async function createBooking() {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        quote_id: active.id,
        booking_number: `BKG-${(active.quote_number || "NEW").replace(
          "Q-",
          ""
        )}`,
        currency: "USD",
        status: "pending",
        total_amount: active.final_price_total || 0,
        paid_amount: 0,
              ...
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <div className="text-sm text-gray-500">
                    {active.destination} • {active.group_size} pax
                  </div>
                </div>
                <div className="flex gap-2">
                  ...
                  >
                    {t("quote.generatePdf")} EN
                  </button>
                  <button
                    className="px-3 py-2 rounded bg-gray-100"
                    onClick={() => generatePdf("he")}
                  >
                    {t("quote.generatePdf")} HE
                  </button>
                  <button
                    className="px-3 py-2 rounded bg-blue-600 text-white"
                    onClick={createBooking}
                  >
                    {t("quote.createBooking")}
                  </button>
                </div>
              </div>
